...
export const PaySalary = (
  ...
  try {
    ...
    const { data } = await axios.post(
      `https://hidden-oasis-02186.herokuapp.com/api/teachers/fees/${teachername}/${teacherid}`,
      {
        salaryForTheYear,
        salaryForTheMonth,
        salaryAmount,
...
export const teacherregister = (
  ...
  try {
    ...
    const { data } = await axios.post(
      'https://hidden-oasis-02186.herokuapp.com/api/teachers/register',
      {
        teacher_name,
...
export const deleteTeacher = (id) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.delete(`https://hidden-oasis-02186.herokuapp.com/api/teachers/delete/${id}`)
    dispatch({
...
export const listTeachers = () => async (dispatch) => {
  try {
    ...
    const { data } = await axios.get('https://hidden-oasis-02186.herokuapp.com/api/teachers')
    dispatch({
