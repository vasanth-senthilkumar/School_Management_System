...
export const PaySalary = (
  ...
  try {
    ...
    const { data } = await axios.post(
      `https://hidden-oasis-02186.herokuapp.com/api/STAFFs/fees/${staffname}/${staffid}`,
      {
        salaryForTheYear,
        salaryForTheMonth,
        salaryAmount,
...
export const staffregister = (
  ...
  try {
    ...
    const { data } = await axios.post(
      'https://hidden-oasis-02186.herokuapp.com/api/staffs/register',
      {
        staff_name,
...
export const deleteStaff = (id) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.delete(`https://hidden-oasis-02186.herokuapp.com/api/staffs/delete/${id}`)
    dispatch({
...
export const listStaffs = () => async (dispatch) => {
  try {
    ...
    const { data } = await axios.get('https://hidden-oasis-02186.herokuapp.com/api/staffs')
    dispatch({
