...
const TeacherRegister = ({ history }) => {
  ...
  const uploadFileHandler = async (e) => {
    const { data: CLOUDINARY_URL } = await axios.get('https://hidden-oasis-02186.herokuapp.com/api/config/cloudinary')

    const { data: CLOUDINARY_UPLOAD_PRESET } = await axios.get(
      'https://hidden-oasis-02186.herokuapp.com/api/config/cloudinarypreset'
    )
    setTime(true)
    setTimeout(() => {
      setTime(false)
